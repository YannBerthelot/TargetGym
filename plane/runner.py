...
def run_constant_policy(power: float, stick: float, env: Airplane2D, params: EnvParams, steps: int = 10000):
    ...
    def step_fn(carry, _):
        ...
        obs, new_state, reward, new_done, info = env.step_env(key, state, action, params)
    ...
    (_, final_state, done), z_history = jax.lax.scan(step_fn, (key, state, False), None, length=steps)
    return final_state.z, z_history


def run_constant_policy_final_alt(power: float, stick: float, env: Airplane2D, params: EnvParams, steps: int = 10000):
    key = jax.random.PRNGKey(0)
    obs, state = env.reset_env(key, params)
    action = (power, stick)
    ...
    (_, final_state, done), _ = jax.lax.scan(step_fn, (key, state, False), None, length=steps)
    return final_state.z


def run_power_stick_grid(power_levels, stick_levels, env, params, steps=10000, save_csv_path=None):
    def run_one_power(power):
        return jax.vmap(lambda s: run_constant_policy_final_alt(power, s, env, params, steps))(stick_levels)
...
def build_power_interpolator_from_csv(csv_path, stick=0.0):
    ...
    if not (np.all(np.diff(altitudes) >= 0) or np.all(np.diff(altitudes) <= 0)):
        raise ValueError(f"Altitude not monotonic for stick={stick}, interpolation ambiguous.")
...
def run_mode(mode: str, power=1.0, stick=0.0):
    ...
    if mode == "2d":
        ...
        def run_vmapped(powers):
            return jax.vmap(lambda p: run_constant_policy(p, stick_level, env, params))(powers)
        ...
        final_alts = run_power_stick_grid(power_levels, stick_levels, env, params, steps=20000, save_csv_path="power_stick_altitudes.csv")
