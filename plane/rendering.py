...
def draw_cloud(surface, center_x, center_y, scale=1.0, seed=42,
               color=(255, 255, 255), outline_color=(0,0,0), outline_thickness=3):
    ...
    def make_cloud_surf(extra_radius, fill_color):
        ...
        for dx, dy, r in circles:
            x = int(surf_w//2 + dx)
            y = int(surf_h//2 + dy)
            pygame.gfxdraw.filled_circle(temp_surf, x, y, int(r + extra_radius), fill_color)
        return temp_surf
...
def render_plane_scene(screen_width, screen_height, state, params, positions_history, cloud_positions, max_steps,ground_buffer=500):
    ...
    passenger_windows_points = rotate_and_translate_shape(passenger_windows_shape,-state.theta, planex, planey)
    ...
    def rotate_and_translate(points):
        return [(rotate_point(x, y, state.theta)[0]+planex,
    ...
    draw_dashed_line(surf, (20, 20, 20), (0, target_y), (screen_width, target_y), 
                    dash_length=10, space_length=10)
    ...
    text_target   = font.render(f"Target Alt: {int(state.target_altitude * 3.281)} ft", True, (255, 0, 0))
    text_altitude = font.render(f"Altitude: {int(state.z * 3.281)} ft - {int(state.z)} m", True, (0, 0, 255))
    text_distance = font.render(f"Distance:  {int(state.x*0.539957/1000)} nm - {int(state.x/1000)} km", True, (0, 0, 255))
    text_velocity = font.render(f"Speed: {int(state.x_dot * 1.944)} kt - {int(state.x_dot * 3.6)} km/h", True, (0, 0, 255))
    text_pitch    = font.render(f"Pitch: {np.rad2deg(state.theta):.1f}°", True, (0, 0, 255))
    text_power    = font.render(f"Power: {state.power*100:.0f}%", True, (0, 0, 255))
    text_stick    = font.render(f"Stick: {np.rad2deg(state.stick):.0f}°", True, (0, 0, 255))
    ...
    if done1:
        ...
        reward = ((max_alt_diff - abs(state.target_altitude - state.z)) / max_alt_diff) ** 2
    ...
    col_spacing = [left_width + padding*2, middle_width + padding*2, right_width + padding*2]
...
def _render(cls , screen, state, params, frames, clock):
    ...
    planey = int(cls.screen_height - (state.z + 500) * cls.screen_height / (params.max_alt + 500))
