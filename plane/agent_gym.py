...
from plane.env_gymnasium import Airplane2D
import tempfile
from typing import Callable

def evaluate_and_save_video(model, env_id, video_folder, seed, env_kwargs, episode_index=0):
...
class TrainingCallback:
    def __init__(self, video_folder: str, eval_freq: int, total_timesteps: int, seed: int, mode: str="power_and_stick"):
    ...
    def __call__(self, locals: dict, globals: dict) -> bool:
        ...
        if self.step_count >= self.eval_freq * (self.video_count+1):
            ...
            print(f"Saved evaluation video at {progress*100:.1f}% progress to {video_path}")
...
def main():
    ...
    for seed in range(10):
        ...
        os.rename(
            final_path,
            os.path.join(video_folder, f"rl-video-{final_name}.mp4")
        )
