...
from plane.env_jax import EnvState, EnvParams
from plane.env_gymnasium import Airplane2D as GymAirplane2D

def are_images_similar(img1: np.ndarray, img2: np.ndarray, threshold: float = 0.95) -> bool:
    """
...
def test_basic_render():
    ...
    frames, screen, clock = jax_env.render(None, state, jax_env.default_params, [], None)
    assert len(frames) == 1
...
def test_render_trajectory():
    ...
    jax_state = EnvState(**{f:getattr(gym_state, f) for f in gym_state.__dataclass_fields__})
    ...
    for _ in range(10):
        ...
        # Step environments
        jax_obs, jax_state, reward, terminated, truncated, info = jax_env.step(key, jax_state, action, jax_env.default_params)
        gym_obs, reward, terminated, truncated, info = gym_env.step(action)

        gym_state = gym_env.state
        
        # Get frames
        jax_frame_list, jax_screen, jax_clock = jax_env.render(jax_screen, jax_state, 
                                                              jax_env.default_params, 
                                                              jax_frames, jax_clock)
        gym_frame = gym_env.render()
...
def test_save_renders():
    ...
    try:
        ...
        file = jax_env.save_video(lambda x: (0.8, 5.0), seed, 
                                folder=temp_dir,
                                episode_index=0)
        ...
        save_video(
            frames,
            temp_dir,
            fps=gym_env.metadata["render_fps"],
            episode_index=0
        )
