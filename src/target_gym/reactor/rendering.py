...
def _draw_vessel(ax, state, params):
    ...
    vessel = mpatches.FancyBboxPatch(
        ...
        fc="#111e2c", ec=_FRAME, lw=2.5,
    )
    ax.add_patch(vessel)

    # Dome top
    dome = mpatches.Wedge(
        (vx + vw / 2, vy + vh - 0.01), vw * 0.43, 0, 180,
        fc="#111e2c", ec=_FRAME, lw=2.5,
    )
    ax.add_patch(dome)
    ...
    cool_rgba = plt.cm.coolwarm(
        float(np.clip((T_cool - 550) / 150, 0, 1))
    )
    ax.add_patch(mpatches.Rectangle(
        (cx, cy), cw, ch, fc=cool_rgba, alpha=0.18, ec="none", zorder=1,
    ))
    ...
    for k in range(8):
        ...
        ax.add_patch(mpatches.Ellipse(
            (0.50, cy + ch * 0.45), r * 2.2, r * 3.5,
            fc=(0.20, 0.50, 1.0), alpha=a, ec="none", zorder=1,
        ))
    ...
    for x in rod_xs:
        ax.add_patch(mpatches.Rectangle(
            (x - rod_w / 2, cy + 0.02), rod_w, ch - 0.04,
            fc=fc, ec="#1a1a1a", lw=0.5, zorder=3,
        ))

    # -- Control rods --
    rho = float(state.rho_ext)
    rod_frac = float(np.clip(
    ...
    for x in ctrl_xs:
        # Shaft
        ax.add_patch(mpatches.Rectangle(
            (x - cw_rod / 2, tip_y), cw_rod, shaft_top - tip_y,
            fc="#282828", ec="#444", lw=0.6, zorder=4,
        ))
        # Absorber tip (darker, wider)
        ax.add_patch(mpatches.Rectangle(
            (x - cw_rod * 0.8, tip_y), cw_rod * 1.6, 0.025,
            fc="#0a0a0a", ec="#555", lw=0.6, zorder=5,
        ))
    ...
    if xe_a > 0.005:
        for dy in (0.0, 0.15, -0.15, 0.30, -0.08):
            ax.add_patch(mpatches.Ellipse(
                (0.50, cy + ch * 0.50 + dy), cw * 0.80, ch * 0.22,
                fc="#7b1fa2", alpha=xe_a * 0.45, ec="none", zorder=2,
            ))
    ...
    ax.annotate(
        "", xy=(0.28, cy + 0.04), xytext=(0.28, cy - 0.06), arrowprops=arr_in,
    )
    ax.annotate(
        "", xy=(0.72, cy + ch + 0.06), xytext=(0.72, cy + ch - 0.04),
        arrowprops=arr_out,
    )
    ax.text(
        0.28, cy - 0.08, "IN",
        ha="center", va="top", fontsize=6, color=_DIM, family="monospace",
    )
    ax.text(
        0.72, cy + ch + 0.08, "OUT",
        ha="center", va="bottom", fontsize=6, color=_DIM, family="monospace",
    )

    # -- Labels --
    ax.text(
        0.50, 1.14, "REACTOR  VESSEL",
        ha="center", va="bottom", fontsize=11, color=_TEXT,
        fontweight="bold", family="monospace",
    )
    ax.text(
        0.93, 0.94, f"RODS\n{rod_frac * 100:.0f}% WDN",
        ha="center", va="top", fontsize=7, color=_DIM, family="monospace",
    )


# -- Horizontal gauge bar ------------------------------------------------------
def _hbar(
    ax, y, frac, color, label, value_str, *,
    limit_frac=None, target_frac=None,
    bar_h=0.065, x0=0.30, bw=0.48,
    bipolar=False, neg_color=None,
):
    ...
    if bipolar:
        ...
        if clamped >= 0:
            ...
            if fw > 0.002:
                ax.add_patch(mpatches.Rectangle(
                    (mid, y + pad_y), fw, inner_h,
                    fc=color, alpha=0.82, ec="none",
                ))
        else:
            fw = (bw / 2) * (-clamped)
            if fw > 0.002:
                ax.add_patch(mpatches.Rectangle(
                    (mid - fw, y + pad_y), fw, inner_h,
                    fc=neg_color or _RED, alpha=0.82, ec="none",
                ))
        # Center zero line
        ax.plot(
            [mid, mid], [y - 0.003, y + bar_h + 0.003],
            color=_DIM, lw=0.8, alpha=0.6,
        )
    else:
        fw = bw * float(np.clip(frac, 0, 1))
        if fw > 0.002:
            ax.add_patch(mpatches.Rectangle(
                (x0 + pad_x, y + pad_y), fw - 2 * pad_x, inner_h,
                fc=color, alpha=0.82, ec="none",
            ))
    ...
    if limit_frac is not None:
        ...
        ax.plot(
            [lx, lx], [y - 0.006, y + bar_h + 0.006],
            color=_RED, lw=1.4, ls="--", alpha=0.75,
        )
    # Target triangle
    ...
    # Label (left)
    ax.text(
        x0 - 0.02, y + bar_h / 2, label,
        ha="right", va="center", fontsize=8, color=_DIM,
        family="monospace", fontweight="bold",
    )
    # Value (right)
    ax.text(
        x0 + bw + 0.02, y + bar_h / 2, value_str,
        ha="left", va="center", fontsize=8.5, color=_TEXT, family="monospace",
    )
...
def _draw_gauges(ax, state, params, cumul_revenue_M=0.0):
    ...
    ax.text(
        0.50, 0.97, "INSTRUMENTS",
        ha="center", va="top", fontsize=11, color=_TEXT,
        fontweight="bold", family="monospace",
    )
    ...
    _hbar(
        ax, y0, np.clip(n / params.n_max, 0, 1), _CYAN, "POWER",
        f"{n:.3f}", limit_frac=1.0,
    ...
    _hbar(ax, y0 - 2 * dy, tf_frac, _AMBER, "T FUEL",
          f"{T_fuel:.0f} K", limit_frac=1.0)

    # 4. Coolant temperature
    tc_frac = float(np.clip((T_cool - 500) / (params.T_coolant_max - 500), 0, 1))
    _hbar(ax, y0 - 3 * dy, tc_frac, "#ff7043", "T COOL",
          f"{T_cool:.0f} K", limit_frac=1.0)

    # 5. Xe-135 (scale 0-3x equilibrium; eq marker at 1/3)
    _hbar(ax, y0 - 4 * dy, float(np.clip(xe / 3.0, 0, 1)), _PURPLE, "Xe-135",
          f"{xe:.2f}", target_frac=1.0 / 3.0)

    # 6. I-135
    _hbar(ax, y0 - 5 * dy, float(np.clip(iod / 3.0, 0, 1)), "#7e57c2", "I-135",
          f"{iod:.2f}", target_frac=1.0 / 3.0)

    # 7. Reward
    _hbar(ax, y0 - 6 * dy, float(np.clip(reward, 0, 1)), _GREEN, "REWARD",
          f"{reward:.2f}")
    ...
    _hbar(ax, y0 - 7 * dy, rev_frac, _GREEN, "CUM. REV",
          f"{cumul_revenue_M:+.2f} M$",
          bipolar=True, neg_color=_RED)

    # Status dot
    scram = (
        n > params.n_max
        or T_fuel > params.T_fuel_max
        or T_cool > params.T_coolant_max
    )
    sc, st = (_RED, "SCRAM") if scram else (_GREEN, "NOMINAL")
    ax.text(
        0.50, y0 - 8 * dy + 0.02, f"\u25cf {st}",
        ha="center", va="center", fontsize=10, color=sc,
        fontweight="bold", family="monospace",
    )
...
def _draw_strip(ax, history, params):
    ...
    ax.plot(
        t_plot, tgt_all,
        color="white", lw=1, ls="--", alpha=0.55, label="target", zorder=2,
    )
    ...
    ax.set_xlabel(
        ...
        fontsize=7, color=_DIM, family="monospace",
    )
    ax.legend(
        loc="upper right", fontsize=6, framealpha=0.4,
        facecolor=_PANEL, edgecolor=_FRAME, labelcolor=_TEXT,
    )
...
def render_reactor(state, params, step, history):
    ...
    fig.text(
        0.50, 0.975, "NUCLEAR  REACTOR  \u2014  CONTROL  ROOM",
        ha="center", va="top", fontsize=14, color=_TEXT,
        fontweight="bold", family="monospace",
    )
    fig.text(
        0.97, 0.975, f"T+{int(hrs):02d}:{int(mins):02d}:{int(secs):02d}",
        ha="right", va="top", fontsize=10, color=_DIM, family="monospace",
    )
    fig.text(
        0.03, 0.975, f"STEP {step}",
        ha="left", va="top", fontsize=10, color=_DIM, family="monospace",
    )
